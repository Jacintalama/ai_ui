"""Create (or reuse) the Discord recruiting channel and post its button panel.
...
Usage (in the repo):
    ...
    [RECRUITING_CHANNEL_ID=<channel-snowflake>] \
    [RECRUITING_CHANNEL_NAME=recruiting] \
    python webhook-handler/scripts/setup_recruiting_channel.py

Usage (inside the deployed webhook-handler container):
    docker compose -f docker-compose.unified.yml exec \
      -e DISCORD_GUILD_ID=... \
      webhook-handler python /app/scripts/setup_recruiting_channel.py
...
from handlers.recruiting_panel import build_recruiting_embed, build_recruiting_panel  # noqa: E402
...
def _create_channel(guild_id: str, name: str, headers: dict) -> str:
    ...
    body = {"name": name, "type": TEXT_CHANNEL,
            "topic": "Find engineers and send outreach with AIUI — use the panel below."}
    with httpx.Client(timeout=30.0) as client:
...
def main() -> int:
    ...
    channel_id = os.environ.get("RECRUITING_CHANNEL_ID", "").strip()
    channel_name = os.environ.get("RECRUITING_CHANNEL_NAME", "recruiting").strip()
    ...
    panel = build_recruiting_panel()
    embed = build_recruiting_embed()
    payload = {**panel, "embeds": [embed]}

    try:
        # Priority 1: use explicit channel ID if provided and non-empty
        if channel_id:
            print(f"Using channel ID from RECRUITING_CHANNEL_ID: {channel_id}")
        else:
            # Fall back to find-or-create by name (requires guild_id)
            if not guild_id:
                print("ERROR: DISCORD_GUILD_ID must be set when RECRUITING_CHANNEL_ID is empty.",
                      file=sys.stderr)
                return 1
    ...
    print("OK — recruiting panel posted and pinned.")
