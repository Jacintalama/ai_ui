"""Create (or reuse) the Discord #graph channel and post its pinned panel.
...
Usage (inside the deployed webhook-handler container):
    docker compose -f docker-compose.unified.yml exec \
      [-e DISCORD_GUILD_ID=...] [-e GRAPH_CHANNEL_NAME=graph] \
      webhook-handler python /app/scripts/setup_graph_channel.py
...
from handlers.graph_panel import build_panel_payload  # noqa: E402
...
def _create_channel(guild_id: str, name: str, headers: dict) -> str:
    body = {"name": name, "type": TEXT_CHANNEL,
            "topic": "Your personal knowledge graph, built from your chats. Use the panel below."}
    with httpx.Client(timeout=30.0) as client:
...
def main() -> int:
    ...
    channel_id = os.environ.get("GRAPH_CHANNEL_ID", "").strip()
    channel_name = os.environ.get("GRAPH_CHANNEL_NAME", "graph").strip()
    ...
    try:
        if channel_id:
            print(f"Using GRAPH_CHANNEL_ID: {channel_id}")
        else:
            if not guild_id:
    ...
    print("OK — graph panel posted and pinned.")
