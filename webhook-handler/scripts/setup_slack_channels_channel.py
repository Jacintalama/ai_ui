"""Create (or reuse) the Slack #channels channel and post its panel.
...
Usage (inside the deployed webhook-handler container):
    docker compose -f docker-compose.unified.yml exec \
      [-e SLACK_CHANNELS_CHANNEL_ID=C0123ABC] [-e SLACK_CHANNELS_CHANNEL_NAME=channels] \
      webhook-handler python /app/scripts/setup_slack_channels_channel.py
...
from handlers.slack_channels_panel import build_channels_blocks  # noqa: E402
...
def main() -> int:
    ...
    channel_id = os.environ.get("SLACK_CHANNELS_CHANNEL_ID", "").strip()
    name = os.environ.get("SLACK_CHANNELS_CHANNEL_NAME", "channels").strip()
    ...
    try:
        if channel_id:
            print(f"Using SLACK_CHANNELS_CHANNEL_ID: {channel_id}")
        else:
        ...
        _call("chat.postMessage", token, channel=channel_id,
              blocks=build_channels_blocks(base_url),
              text="Channels — talk to IO from Telegram, a terminal and more.")
    ...
    print(f"OK — channels panel posted to #{name} ({channel_id}).")
    return 0
