#!/usr/bin/env python3
"""Install (or re-install) the "Auto (Free)" router pipe in Open WebUI.

A pipe registers a selectable model. Once active + global, every user sees
"Auto (Free)" in the model dropdown; picking it runs the keyword router in
open-webui-functions/auto_router_pipe.py, which forwards to a free OpenRouter
model. Needs OPENROUTER_API_KEY in the open-webui container environment.

Mirrors insert_fusion_action.py. Run from /root/proxy-server on the box:
    docker run --rm --network proxy-server_backend -v /root/proxy-server:/work \
      ...
      'pip install -q psycopg2-binary && DATABASE_URL=postgresql://openwebui:$POSTGRES_PASSWORD@postgres:5432/openwebui \
       python scripts/insert_auto_router_pipe.py'
"""
import json
import os
import pathlib
...
META = {
    "description": "Reads your question and routes it to the best free model.",
    "manifest": {
        "title": FUNC_NAME,
        "author": "Ralph Benitez",
        "version": "1.0.0",
        "description": "Reads your question and routes it to the best free "
                       "model on OpenRouter. No paid model is used.",
    },
}
...
def main() -> int:
    ...
    cur.execute(
        ...
        """,
        (FUNC_ID, user_id, FUNC_NAME, content, json.dumps(META),
         now, now, "{}"),
    )
    conn.commit()
