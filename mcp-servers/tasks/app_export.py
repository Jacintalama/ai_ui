...
# Functions and triggers were missing until 2026-08-03. The roles recipe the
# builder emits (claude_executor.py SUPABASE_SQL_TOOL_TEMPLATE) puts an app's
# entire access-control model inside `public.is_admin()` plus two triggers, and
# policies call it — so a dump without them names a function it never defines
# and refuses to replay. pg_get_functiondef/pg_get_triggerdef hand back the
# exact statement Postgres would need, quoting and body included.
#
# The pg_depend clause drops objects owned by an extension (pgcrypto, uuid-ossp
# and friends install into public on Supabase); those come back with the
# extension, and dumping them would bury the app's own two functions.
_Q_FUNCTIONS = (
...
# 'auth' is deliberately in scope: the signup trigger that makes the first user
# an admin lives on auth.users, not on a public table. Filtering to public
# alone would drop the single most important trigger in the recipe.
_Q_TRIGGERS = (
...
async def _read_or_disclose(run_sql, query: str, what: str) -> tuple[list, str]:
    ...
    Everything else in the export fails open, which is right for a build step.
    It is wrong here: a schema.sql missing its functions still looks complete,
    and the user only discovers otherwise when the restore errors. So a refused
    read becomes a warning inside the artifact itself.
    """
    try:
        return list(await run_sql(query) or []), ""
    except Exception as exc:  # noqa: BLE001 - disclosed, not swallowed
        logger.warning("export: %s introspection failed: %s", what, exc)
        return [], (
            f"-- WARNING: {what} could not be read from the database "
            f"({type(exc).__name__}). Any policy or trigger below that calls "
            f"one will NOT replay until you add it by hand.\n\n"
        )
...
async def build_schema_sql(run_sql) -> str:
    ...
    header = (
        ...
        "-- Columns, RLS state, policies, functions and triggers are reported\n"
        "-- as fact. Statements are ordered so the file replays top to bottom:\n"
        "-- tables, then functions, then RLS and policies, then triggers.\n"
    ...
    functions, fn_warning = await _read_or_disclose(run_sql, _Q_FUNCTIONS, "functions")
    triggers, tg_warning = await _read_or_disclose(run_sql, _Q_TRIGGERS, "triggers")

    out = [header]
    if fn_warning:
        out.append(fn_warning)
    if tg_warning:
        out.append(tg_warning)
    if non_tables:
        out.append(
            "-- Skipped (not ordinary tables, e.g. views): "
            + ", ".join(f'"{t}"' for t in non_tables) + "\n\n")

    # Phase 1: tables. Functions may read them (is_admin() selects from
    # profiles) and LANGUAGE sql bodies are validated at CREATE time, so
    # tables cannot come later.
    ...
    # Phase 2: functions, before any policy or trigger that calls one.
    # pg_get_functiondef returns a complete CREATE OR REPLACE statement with no
    # trailing semicolon.
    for fn in functions:
        definition = (fn.get("definition") or "").rstrip().rstrip(";")
        if definition:
            out.append(definition + ";\n\n")

    # Phase 3: RLS and policies.
    ...
    # Phase 4: triggers, last, since each names a function from phase 2.
    if triggers:
        out.append("-- Triggers. A trigger on auth.users needs elevated rights;\n"
                   "-- run these from the Supabase SQL editor.\n")
        for tg in triggers:
            definition = (tg.get("definition") or "").rstrip().rstrip(";")
            if definition:
                out.append(definition + ";\n")
        out.append("\n")
    return "".join(out)
...
async def export_app(slug: str, *, actor_email: str,
    ...
    try:
        ...
        # The return code of this commit used to be discarded, so a failure
        # shipped a zip whose files exist on disk but are untracked: `git
        # status` dirty, and the filename carrying the pre-export SHA. Never
        # fail the export over it — the app and its history are already in the
        # bundle — but never stay silent about it either.
        rc, out = await _run_git("add", "--", ".", cwd=str(repo))
        if rc != 0:
            logger.warning("export: git add failed for %s (rc=%s): %s",
                           slug, rc, out.strip()[:300])
        else:
            rc, out = await _run_git(
            ...
            if rc != 0:
                logger.warning(
                    "export: commit of README/config/schema failed for %s "
                    "(rc=%s): %s — the bundle ships them uncommitted",
                    slug, rc, out.strip()[:300])
