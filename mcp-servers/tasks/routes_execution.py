...
async def stream(
    ...
    # Stream while the task is actively executing AI work. "planning" is
    # included so users get live tool-output during plan generation AND so
    # the panel learns about the planning -> awaiting_plan_review flip
    # immediately (the SSE close fires refreshAll on the client).
    ACTIVE_STATES = ("running", "planning")
