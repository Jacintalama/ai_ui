...
async def caddy_on_demand_ask(domain: str = ""):
    """Caddy on-demand TLS gatekeeper. Returns 200 when:
    - <slug>.<AIUI_PUBLIC_BASE_URL host> and a published_apps row for slug exists
      (platform-default subdomain — self-serve, no per-app DNS verification needed
      since the wildcard DNS at *.<aiui_host> already covers it).
    - <slug>.<parent> with a published_apps row whose custom_domain == parent
      AND custom_domain_verified_at is set (custom-domain flow).
    Blocks Let's Encrypt rate-limit abuse via random hostnames."""
    ...
    slug, parent = parsed
    aiui_host = (_urlparse(_os.environ.get("AIUI_PUBLIC_BASE_URL", "")).hostname or "").lower()
    async with _db_session() as s:
        if aiui_host and parent == aiui_host:
            row = (await s.execute(
...
async def _supabase_inject_for(slug: str) -> str:
    ...
    return (
        ...
        f"window.AIUI_SLUG={slug_js};"
        "</script>"
    )


def _inject_into_head(html: str, snippet: str) -> str:
    """Insert ``snippet`` immediately after the opening ``<head>`` tag.

    Falls back to prepending if no ``<head>`` is found. Idempotent: if the
    snippet's marker already appears in the body, leave it alone.
    """
    if not snippet:
        return html
    if "window.SUPABASE_URL" in html:
        return html  # already injected (e.g. app code loads aiui-config.js)
    lower = html.lower()
    head_idx = lower.find("<head>")
    if head_idx >= 0:
        return html[: head_idx + 6] + snippet + html[head_idx + 6 :]
    return snippet + html


@app.get("/__preview_root", include_in_schema=False)
async def preview_root_html(request: Request) -> Response:
    """Proxy the running preview's root HTML and inject Supabase config.

    Works as a defense-in-depth backstop for apps that were built without
    ``<script src="aiui-config.js"></script>`` in their ``<head>`` — Caddy
    routes ``/tasks/preview-app/`` and ``/tasks/preview-app/index.html``
    here, we fetch the dev server, splice ``window.SUPABASE_URL`` /
    ``SUPABASE_ANON_KEY`` after ``<head>``, and serve the rewritten HTML.
    Asset paths (CSS / JS / images) still go directly to the dev server
    via the wildcard handler ``/tasks/preview-app/*``.
    """
    ...
    if not cur or not cur.get("slug"):
        return Response(
            ...
            "<p>Click <em>Run</em> in the app builder to start the dev server.</p>",
            status_code=503,
            media_type="text/html",
            headers={"Cache-Control": "no-store"},
        )
    slug = cur["slug"]
    port = cur.get("port", 9100)
    try:
        async with _httpx.AsyncClient(timeout=8.0, follow_redirects=True) as client:
            r = await client.get(f"http://localhost:{port}/")
    except Exception as exc:
        return Response(
            f"<h1>Preview server not reachable</h1><pre>{exc}</pre>",
            status_code=502,
            media_type="text/html",
            headers={"Cache-Control": "no-store"},
        )
    ...
    body = _inject_into_head(body, snippet)
    return Response(
        content=body,
        media_type="text/html; charset=utf-8",
        status_code=r.status_code if r.status_code != 200 else 200,
        headers={"Cache-Control": "no-store"},
    )
