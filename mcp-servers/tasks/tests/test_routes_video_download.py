...
async def test_download_admin_member_authorized(db_session, tmp_path):
    ...
    can download a finished job. The member path authorizes the OWNER or an admin,
    so the admin header is sufficient here (no project role)."""
    out = tmp_path / "out.mp4"
    out.write_bytes(b"\x00\x00\x00\x18ftypmp42fake-mp4-bytes")
    job_id = uuid.uuid4()
    db_session.add(
...
async def test_download_non_admin_member_rejected(db_session, tmp_path):
    """A logged-in NON-admin (X-User-Admin: false) with no capability who does NOT
    own the job is rejected with 403: the member path authorizes the OWNER or an
    admin, not a project role."""
    out = tmp_path / "out.mp4"
    out.write_bytes(b"\x00\x00\x00\x18ftypmp42fake-mp4-bytes")
    job_id = uuid.uuid4()
    db_session.add(
...
@pytest.mark.skipif(not _HAVE_DB, reason="needs Postgres (runs at deploy/CI)")
async def test_download_non_admin_owner_authorized(db_session, tmp_path):
    """A logged-in NON-admin (X-User-Admin: false) who OWNS the job can download
    it (200): the member path authorizes the OWNER, locking in the
    member_email == job.user_email branch."""
    out = tmp_path / "out.mp4"
    out.write_bytes(b"\x00\x00\x00\x18ftypmp42fake-mp4-bytes")
    job_id = uuid.uuid4()
    db_session.add(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://t") as c:
        r = await c.get(
            f"/api/video-jobs/{job_id}/download",
            headers={"X-User-Email": "owner@x.com", "X-User-Admin": "false"},
        )
    assert r.status_code == 200
    assert r.headers["content-type"].startswith("video/mp4")
